...
Looks complicated? Becomes pretty simple, when you look at a plot:
...
The result of `kriging::kriging` is in a tall format -- we have to make it wide to plot it. For this purpose we use `quantaar::spatialwide`.
...
We can also visualize the generated surfaces:
...
Now imagine the excavation leader is away just for thirty minutes eating ice-cream and somebody excavated in this trench in a mind-bogglingly stupid way. Whoever did this managed to take out a strangly shaped spit, took no measurements for single finds and only collected what he found. The only thing we can do now, is taking measurements of the weird spit. 
...
Let's fill the shape with an equidistant three dimensional point raster using `quantaar::fillhexa` and take a look at it.
...
Damn! Whoever did this penetrated all three major horizons with his spit.
